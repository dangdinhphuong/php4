...
<div class="account__wrapper">
    <div class="account__content">
        <h2 class="account__content--title h3 mb-20">Lịch sử mua hàng</h2>
                        ...
                        <div class="col mb-30 "
                            ...
                            <div class="product__items product__list--items d-flex">
                                <div class="product__items--thumbnail product__list--items__thumbnail"
                                    ...
                                    <a class="product__items--link"
                                        href="{{route('order_detail',['id'=>$Order->id])}}">

                                        <img class="product__items--img product__primary--img"
                                            src="{{asset('storage/' .$orderDetail->image)}}"
                                            alt="product-img">
                                    </a>
                                </div>
                                <div class="product__list--items__content">
                                    <h3 class="product__list--items__content--title h4 mb-10"><a
                                            href="{{route('order_detail',['id'=>$Order->id])}}">{{$orderDetail->name}}
                                        </a>
                                    </h3>
                                    <div class="product__list--items__price mb-10">
                                        <span><b>x{{$orderDetail->quantity}}</b></span>
                                        <span
                                            class="current__price">{{ number_format($orderDetail->price * $orderDetail->quantity, 0, ',', '.') . " VNĐ"   }}</span>
                                    </div>
                                </div>
                            </div>
                            <hr>
                            <ul class="product__items--action d-flex">
                                <a class="blog__content--btn primary__btn mr-1"
                                    ...
                                    href="{{ route('product',['slug'=>$orderDetail->slug])  }}">Mua
                                    lại</a>
                                <a class="blog__content--btn primary__btn"
                                    href="{{route('order_detail',['id'=>$Order->id])}}">Xem chi
                                    tiết</a>
                            </ul>
                        </div>
                        @endforeach
