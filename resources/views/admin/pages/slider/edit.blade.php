...
<div class="card">
    <div class="card-header">
       <a href="{{ route('cp-admin.slider.index') }}" class="btn btn-primary">Quay lại</a>
    </div>
    <div class="card-body">
        <form action="{{ route('cp-admin.slider.update',['id'=>$sliders->id] )}}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <option value="{{$link->slug}}" {{ $sliders->link==$link->slug?"selected":"" }}>{{$link->namePro}}</option>
                @endforeach
               </select>
            </div>
