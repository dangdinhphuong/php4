...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{route('cp-admin.products.index')}}"
           class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"> Danh sách sản phẩm</a>
    </div>

    <div class="card shadow mb-4 ">
        <div class="card-body">
            <div class="table-responsive">
                <form class="user" action="{{ route('cp-admin.products.update',[ 'id' => $Product->id ]) }}"
                    ...
                    <div class="form-group row">
                        <div class="col-sm-6 mb-3 mb-sm-0">
                            ...
                            <input type="text" class="form-control form-control-user" id="namePro"
                                   onchange="ChangeToSlug('namePro','slugs')" value="{{ $Product->namePro }}"
                                   name="namePro" placeholder="Tên sản phẩm ...">
                            @error('namePro')
                        ...
                        <div class=" col-sm-6 ">
                            ...
                            <select class="custom-select" name="category_id" id="inputGroupSelect01">
                                ...
                                @foreach($categoryAll as $cate)
                                    <option
                                        value="{{$cate->id}}" {{ $Product->category_id == $cate->id ? "selected": "" }}>{{$cate->nameCate}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class=" col-sm-6 ">
                            ...
                            <select class="custom-select" name="supplier_id" id="inputGroupSelect01">
                                ...
                                @foreach($supplier as $cate)
                                    <option
                                        value="{{$cate->id}}" {{ $Product->supplier_id == $cate->id ? "selected": "" }}>{{$cate->nameSupplier}} {{$cate->status==1? " ": "(Ngưng cung cấp)"}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class=" col-sm-6 ">
                            ...
                            <select class="custom-select" name="origin_id" id="inputGroupSelect01">
                                ...
                                @foreach($origin as $cate)
                                    <option
                                        value="{{$cate->id}}" {{ $Product->origin_id == $cate->id ? "selected": "" }}>{{$cate->name}}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-12 mb-3 mb-sm-0"><label for="slugCategories">Anh sản phẩm<span
                            ...
                            <input onchange="previewFile(this)" id="product_image" type="file" id="image" name="image"
                                   class="form-control" require>
                            <div class="d-flex justify-content-start">
                                <div class="mt-3"
                                     style="width: 45%; height: 200px; border: 1px solid gray; background-color: pink !important;text-align: center;">
                                    <img style="width: 100%; height: 100%;" src="{{ asset("storage/$Product->image") }}"
                                         alt=" Anh cũ">
                                </div>
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12 mb-3 mb-sm-0">
                            <label for="product_images">Ảnh sản phẩm <span class="text-danger">(*)</span></label>
                            <input id="product_images" type="file" name="images[]" multiple class="form-control" onchange="previewMultipleFiles(this)">

                            <!-- Hiển thị ảnh cũ -->
                            <div class="mt-3 d-flex flex-wrap">
                                @if ($Product->images && count($Product->images) > 0)
                                    @foreach ($Product->images as $image)
                                        <div style="width: 100px; height: 100px; margin: 5px; border: 1px solid gray; position: relative;">
                                            <img style="width: 100%; height: 100%; object-fit: cover;" src="{{ asset("storage/$image->image_path") }}" alt="Ảnh cũ">
                                        </div>
                                    @endforeach
                                @else
                                    <p>Không có ảnh cũ</p>
                            ...
                            <div class="mt-3 d-flex flex-wrap" id="previewContainer">
                                <!-- Preview ảnh mới sẽ xuất hiện ở đây -->
                            </div>
                    ...
                    <div class="form-group row ">
                        <div class=" col-sm-12 ">
                            ...
                            <textarea class="form-control" name="Description" id="summernote"
                                      rows="10">{{ isset($Product->Description) ? $Product->Description : 0 }}</textarea>
                        </div>
